...
JSON_RESULT_FILE = "5fold_results_ablation_no_privilege.json"
TXT_LOG_FILE = "5fold_ablation_log.txt"
...
def init_log_file():
    with open(TXT_LOG_FILE, "w") as f:
        f.write(f"🔥 5-Fold Ablation (Start: {time.strftime('%Y-%m-%d %H:%M:%S')})\n")
...
def log_final_stats(stats):
    with open(TXT_LOG_FILE, "a") as f:
        f.write("\n" + "="*140 + "\n")
        f.write("🏆 FINAL STATS (Mean ± Std [Var])\n")
        f.write("="*140 + "\n")
...
def run_fold(fold_idx, train_idx, val_idx, df, toks):
    print(f"\n🔄 [Fold {fold_idx+1}/5] Training (Ablation)...")
...
if __name__ == "__main__":
    init_log_file()
    print(f"🔥 A40 5-FOLD ABLATION (No Privilege) | Batch={BATCH_SIZE}")
    ...
    print("\n📊 Final Stats (Ablation - No Comments):")
    for k in keys:
