...
JSON_RESULT_FILE = "5fold_results_ablation_no_agent.json"
TXT_LOG_FILE = "5fold_ablation_no_agent_log.txt"
...
BEST_PARAMS = {"lr": 2e-5, "alpha": 0.5, "temp": 2.0}
...
def init_log_file():
    with open(TXT_LOG_FILE, "w") as f:
        f.write(f"🔥 5-Fold Ablation (NO AGENT) (Start: {time.strftime('%Y-%m-%d %H:%M:%S')})\n")
        f.write(f"📌 Params: {BEST_PARAMS}\n")
        f.write(f"📌 Strategy: Static Average Fusion (0.5 * T1 + 0.5 * T2)\n")
        f.write("="*140 + "\n")
        header = (f"{'Fold':<6} | {'Macro F1':<10} | {'Acc':<8} | {'AUC':<8} | "
                  f"{'F1 Real':<8} | {'F1 Fake':<8} | "
                  f"{'Prec Real':<9} | {'Prec Fake':<9} | "
                  f"{'Rec Real':<9} | {'Rec Fake':<9}\n")
        f.write(header)
        f.write("="*140 + "\n")
...
def log_fold_result(fold, metrics):
    with open(TXT_LOG_FILE, "a") as f:
        line = (f"{fold:<6} | {metrics['macro_f1']:.4f}     | {metrics['accuracy']:.4f}   | {metrics['AUC']:.4f}   | "
                f"{metrics['f1_real']:.4f}   | {metrics['f1_fake']:.4f}   | "
                f"{metrics['precision_real']:.4f}    | {metrics['precision_fake']:.4f}    | "
                f"{metrics['recall_real']:.4f}    | {metrics['recall_fake']:.4f}\n")
        f.write(line)

def log_final_stats(stats):
    with open(TXT_LOG_FILE, "a") as f:
        f.write("\n" + "="*140 + "\n")
        f.write("🏆 FINAL STATS (No Agent)\n")
        f.write("="*140 + "\n")
        order = ['macro_f1', 'accuracy', 'AUC', 'f1_real', 'f1_fake',
                 'precision_real', 'precision_fake', 'recall_real', 'recall_fake',
...
def run_fold(fold_idx, train_idx, val_idx, df, toks):
    print(f"\n🔄 [Fold {fold_idx+1}/5] Training (No Agent)...")
    ...
    for ep in range(EPOCHS):
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast('cuda'):
                ...
                weighted_teacher = torch.mean(t_stack, dim=1)
    ...
    del train_loader, val_loader, student, teachers, opt_s, scaler
...
if __name__ == "__main__":
    init_log_file()
    print(f"🔥 A40 5-FOLD ABLATION (No Agent) | Batch={BATCH_SIZE}")
    ...
    print("\n📊 Final Stats (No Agent):")
    for k in keys:
        values = [r[k] for r in fold_results]
        final_stats[k] = {"mean": np.mean(values), "std": np.std(values), "var": np.var(values)}
    ...
    print("\n✅ Ablation Study (No Agent) Completed!")
